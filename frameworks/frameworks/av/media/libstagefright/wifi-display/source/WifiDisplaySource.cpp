...
WifiDisplaySource::WifiDisplaySource(
    ...
    mSupportedSourceVideoFormats.enableResolutionUpto(
            VideoFormats::RESOLUTION_CEA, 7, // 7 : 1920x1080p30 8 : 1920x1080p60
            VideoFormats::PROFILE_CHP,  // Constrained High Profile
            VideoFormats::LEVEL_32);    // Level 3.2
}

WifiDisplaySource::~WifiDisplaySource() {
	ALOGI("~WifiDisplaySource");
}

static status_t PostAndAwaitResponse(
        const sp<AMessage> &msg, sp<AMessage> *response) {
	ALOGI("PostAndAwaitResponse");
    ...
    switch (msg->what()) {
        case kWhatStart:
        {
            ...
            if (err == OK) {
                if (inet_aton(iface.c_str(), &mInterfaceAddr) != 0) {
                    ...
                    ALOGI("createRTSPServer - port : %d / iface : %s",port,iface.c_str());
        ...
        case kWhatRTSPNotify:
        {
            ...
            switch (reason) {
                ...
                {
                	ALOGI("ANetworkSession::kWhatError");
                    int32_t sessionID;
                ...
                {
                	ALOGI("ANetworkSession::kWhatClientConnected");
                    int32_t sessionID;
                    CHECK(msg->findInt32("sessionID", &sessionID));

                    if (mClientSessionID > 0) {
                        ALOGI("A client tried to connect, but we already "
                              "have one.");
                ...
                {
                	ALOGI("ANetworkSession::kWhatData");
                ...
                {
                	ALOGI("ANetworkSession::kWhatNetworkStall");
                    break;
                }
        ...
        {
        	ALOGI("kWhatPlaybackSessionNotify");
            ...
            if (what == PlaybackSession::kWhatSessionDead) {
            	ALOGI("klaybackSession::kWhatSessionDead");
                ALOGI("playback session wants to quit.");

                mClient->onDisplayError(
                        IRemoteDisplayClient::kDisplayErrorUnknown);
            } else if (what == PlaybackSession::kWhatSessionEstablished) {
            	ALOGI("klaybackSession::kWhatSessionEstablished");
            	...
            	ALOGI("klaybackSession::kWhatSessionDestroyed");
                disconnectClient2();
            } else {
            	ALOGI("klaybackSession::else");
        ...
        {
        	ALOGI("kWhatTeardownTriggerTimedOut");
        ...
        case kWhatHDCPNotify:
        {
            ...
            switch (msgCode) {
                ...
                {
                	ALOGI("HDCPModule::HDCP_INITIALIZATION_COMPLETE");
                    mHDCPInitializationComplete = true;

                    if (mSetupTriggerDeferred) {
                    	ALOGI("HDCPModule::mSetupTriggerDeferred");
                        mSetupTriggerDeferred = false;
                ...
                case HDCPModule::HDCP_SHUTDOWN_COMPLETE:
                	ALOGI("HDCPModule::HDCP_SHUTDOWN_COMPLETE");
                case HDCPModule::HDCP_SHUTDOWN_FAILED:
                {
                	ALOGI("HDCPModule::HDCP_SHUTDOWN_FAILED");
...
status_t WifiDisplaySource::onReceiveM16Response(
        int32_t sessionID, const sp<ParsedMessage> &msg) {
	ALOGI("onReceiveM16Response");
    // If only the response was required to include a "Session:" header...

    CHECK_EQ(sessionID, mClientSessionID);

    if (mClientInfo.mPlaybackSession != NULL) {
    	ALOGI("mClientInfo.mPlaybackSession->updateLiveness()");
...
status_t WifiDisplaySource::onReceiveClientData(const sp<AMessage> &msg) {
    ...
    if (method.startsWith("RTSP/")) {
        ...
        if (index < 0) {
            ALOGI("Received unsolicited server response, cseq %d", cseq);
            return ERROR_MALFORMED;
        }
        ...
        if (err != OK) {
            ALOGI("Response handler for session %d, cseq %d returned "
                  "err %d (%s)",
...
status_t WifiDisplaySource::onPlayRequest(
    ...
    if (mState != AWAITING_CLIENT_PLAY
     && mState != PAUSED_TO_PLAYING
     && mState != PAUSED) {
        ALOGI("Received PLAY request but we're in state %d", mState);
...
int32_t WifiDisplaySource::makeUniquePlaybackSessionID() const {
	ALOGI("makeUniquePlaybackSessionID");
    return rand();
}
