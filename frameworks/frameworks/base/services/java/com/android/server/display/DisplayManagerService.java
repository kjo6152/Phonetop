...
public final class DisplayManagerService extends IDisplayManager.Stub {
    ...
    private UsbDisplayAdapter mUsbDisplayAdapter;
    ...
    @Override // Binder call
    public void connectUsbDisplay(String address) {
        if (address == null) {
            throw new IllegalArgumentException("address must not be null");
        }
        mContext.enforceCallingOrSelfPermission(Manifest.permission.CONFIGURE_WIFI_DISPLAY,
        ...
        try {
            synchronized (mSyncRoot) {
                if (mUsbDisplayAdapter != null) {
                	mUsbDisplayAdapter.requestConnectLocked(address);
                }
            }
        } finally {
            Binder.restoreCallingIdentity(token);
    ...
    public void pauseUsbDisplay() {
        ...
        try {
            synchronized (mSyncRoot) {
                if (mUsbDisplayAdapter != null) {
                	mUsbDisplayAdapter.requestPauseLocked();
                }
            }
        } finally {
            Binder.restoreCallingIdentity(token);
    ...
    public void resumeUsbDisplay() {
        ...
        try {
            synchronized (mSyncRoot) {
                if (mUsbDisplayAdapter != null) {
                	mUsbDisplayAdapter.requestResumeLocked();
                }
            }
        } finally {
            Binder.restoreCallingIdentity(token);
    ...
    public void disconnectUsbDisplay() {
        ...
        try {
            synchronized (mSyncRoot) {
                if (mUsbDisplayAdapter != null) {
                	mUsbDisplayAdapter.requestDisconnectLocked();
                }
            }
        } finally {
            Binder.restoreCallingIdentity(token);
    ...
    private void registerAdditionalDisplayAdapters() {
        synchronized (mSyncRoot) {
            if (shouldRegisterNonEssentialDisplayAdaptersLocked()) {
                ...
                registerUsbDisplayAdapterLocked();
    ...
    private void registerUsbDisplayAdapterLocked() {
    	Slog.d(TAG, "registerUsbDisplayAdapterLocked");
             mUsbDisplayAdapter = new UsbDisplayAdapter(
                    mSyncRoot, mContext, mHandler, mDisplayAdapterListener,
                    mPersistentDataStore);
            registerDisplayAdapterLocked(mUsbDisplayAdapter);
    }
