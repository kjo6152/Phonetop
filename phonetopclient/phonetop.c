...
static void event_loop(VideoState *cur_stream)
{
    ...
    double show,min,max,ret_x,ret_y,start_x,start_y;
    for (;;) {
        double x;
...
static void sighandler(int signum){
	if(signum==SIGRTMIN){
		printf("signum : %d",SIGRTMIN);
		rotation = 0;
	}else if(signum==SIGRTMIN+1){
		printf("signum : %d",SIGRTMIN+1);
		rotation = 1;
	}
}
