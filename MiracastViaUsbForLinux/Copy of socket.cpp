...
unsigned char TSPacketBuffer[4096];
...
void ReceiveRtpData(int socket) {
	...
	int fd = open("RtpData", O_WRONLY | O_CREAT);
	if (fd < 0)
		return;

	while (true) {
		cout << "in while"<< endl;
		ret = ReadTSData(socket);
		if(ret){
			write(fd, TSPacketBuffer, 188);
		}
	}

}

int ReadTSData(int socket) {
	struct sockaddr_in client_addr;
	unsigned int client_addr_len = sizeof(client_addr);
	int ret;
	ret = recvfrom(socket, TSPacketBuffer, 12, 0, (struct sockaddr *) &client_addr,	&client_addr_len);
	cout << "ret : "<< ret << endl;
	if(TSPacketBuffer[0]==0x80 && TSPacketBuffer[1]==0x21){
		cout << "Receive Header"<< endl;
		return 0;
	}else {
		cout << "Receive TS Packet"<< endl;
		recvfrom(socket, TSPacketBuffer+12, 176, 0, (struct sockaddr *) &client_addr,	&client_addr_len);
	}
	return 1;
}
